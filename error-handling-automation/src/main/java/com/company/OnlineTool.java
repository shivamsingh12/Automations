...
public class OnlineTool {
    ...
    public static OnlineTool getOnlineTool(WebDriver driver) throws Exception{
        ...
        onlineTool.waitShort = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(5)).pollingEvery(Duration.ofMillis(300)).ignoreAll(List.of(NoSuchElementException.class, ElementNotInteractableException.class, StaleElementReferenceException.class));
        return onlineTool;
    }
...
//        System.out.println("currently in getAllInHandling");
        ...
        System.out.println("perfromActionOnAll : action of "+actionEnum);
...
//            transactions = driver.findElements(By.className("selected"));
    ...
    private boolean tryClick(WebElement webElement) throws Exception{
        System.out.println("currently in tryCick");
        ...
        for(int i = 0; i < 3; i++){
            try {
                ...
                dialogs = driver.findElements(By.xpath("/html/body/div[5]/div[3]/div/button[1]"));
                overlays = driver.findElements(By.id("overlay"));
                if(dialogs.size()>0){
                    dialogs.get(0).click();
                    break;
...
//        id.click();
    ...
    private enum Window{
        SUB_SERVICE, ERROR_HANDLER, DEVICE_Q
    }
